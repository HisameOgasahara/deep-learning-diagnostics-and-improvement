...
from vit_lab_model_optim import SmallViT, TRACKED_PARAM_NAMES, OptimizerBundle, make_optimizer
...
class GradientNoiseAccumulator:
    ...
    def add(self, named_params):
        ...
        for name in TRACKED_PARAM_NAMES:
            g = named_params[name].grad
            if g is None:
                continue
            g_cpu = g.detach().float().cpu()
            if name not in self.sum_grad:
                self.sum_grad[name] = torch.zeros_like(g_cpu)
            self.sum_grad[name].add_(g_cpu)
            self.sum_sq_norm[name] += g_cpu.square().sum().item()

    def summarize(self):
        out = {}
        if self.count == 0:
            return out
        for name, sum_g in self.sum_grad.items():
            mean_g = sum_g / self.count
            mean_sq_norm = self.sum_sq_norm[name] / self.count
            signal = mean_g.square().sum().item()
            variance_trace = max(0.0, mean_sq_norm - signal)
            out[name] = {
                "grad_mean_norm": math.sqrt(signal),
                "grad_variance_trace": variance_trace,
                "gradient_noise_ratio": variance_trace / (signal + 1e-12),
            }
        return out
...
def train_one_optimizer(
    ...
    history_rows, dynamics_rows, noise_rows = [], [], []
    named_params = dict(model.named_parameters())
    initial_tracked = {
        name: named_params[name].detach().cpu().clone()
        for name in TRACKED_PARAM_NAMES
    }
    previous_grad, previous_update = {}, {}
    global_step = 0

    torch.save(model.state_dict(), ckpt_dir / f"{run_name}_epoch0.pt")
        ...
        for x, y in train_loader:
            ...
            if should_track:
                ...
                for param_name in TRACKED_PARAM_NAMES:
                    p = named_params[param_name]
                    before_update[param_name] = p.detach().clone()
                    g = p.grad.detach()
                    dynamics_rows.append({
                        ...
                        "grad_cosine_prev": (
                            cosine_flat(g, previous_grad[param_name])
                            if param_name in previous_grad else np.nan
                        ),
                    })
                    previous_grad[param_name] = g.clone()
            ...
            if should_track:
                named_params = dict(model.named_parameters())
                recent = dynamics_rows[-len(TRACKED_PARAM_NAMES):]
                for row in recent:
                    param_name = row["parameter"]
                    p = named_params[param_name]
                    update = p.detach() - before_update[param_name]
                    row["update_norm"] = update.norm().item()
                    row["update_to_weight"] = (
                        update.norm() / (p.detach().norm() + 1e-12)
                    ).item()
                    row["update_cosine_prev"] = (
                        cosine_flat(update, previous_update[param_name])
                        if param_name in previous_update else np.nan
                    )
                    row["parameter_displacement"] = (
                        p.detach().cpu() - initial_tracked[param_name]
                    ).norm().item()
    ...
    noise_df.to_csv(csv_dir / f"{run_name}_gradient_noise.csv", index=False)
