...
그런데 `02`가 중요한 반례를 줬어. 성공 세팅에서 **약 1027 update만 해도 val 0.652**, 약 2054 update에서는 0.728이었어. 실패 세팅은 약 1950 update인데 0.467이었고. 따라서

\[
\boxed{\text{“step 수가 절반이라 실패했다”는 설명은 틀림}}
\]
...
\[
CKA_{\text{init}}: 0.313 \rightarrow 0.168
\]

\[
\text{linear probe}: 0.4565 \rightarrow 0.7558
\]
...
\[
NC1: 4.01 \rightarrow 1.62
\]

\[
kNN\ purity: 0.295 \rightarrow 0.686
\]

\[
margin: -0.179 \rightarrow +2.55
\]
...
\[
\boxed{
\text{실패 세팅에서는 클래스별 feature manifold가 제대로 untangle되지 않았다}
}
\]
...
\[
\|J\|_2 = 59.9,\qquad r_J = 1.82
\]

복원:

\[
\|J\|_2 = 10.9,\qquad r_J = 4.32
\]
...
\[
\lambda_{\min}(H)\simeq -10\,353,\qquad
\lambda_{\max}(H)\simeq16\,827
\]
...
\[
\text{Muon이 경험하는 stochastic gradient field 변화}
\]
...
\[
\text{feature는 움직이지만 class-separable하게 조직되지 않음}
\]
...
\[
\text{함수 민감도가 소수 방향에 집중}
\quad
(\|J\|_2\uparrow,\ r_J\downarrow)
\]
...
\[
\text{train accuracy 자체가 44\%에서 정체}
\]
...
\[
\boxed{
\text{그 방향을 안정적인 feature geometry로 만들어내는 과정이 실패}
}
\]
...
\[
\text{representation organization}
+
\text{Jacobian geometry}
+
...
\text{원인 후보}
\rightarrow
\boxed{\text{gradient/update dynamics}}
\rightarrow
\boxed{\text{representation}}
\rightarrow
\boxed{\text{function/landscape}}
\rightarrow
\text{accuracy}
\]

에서 가운데 **representation 이후는 매우 잘 잡았는데**, `N,B → gradient/update dynamics` 부분의 직접 증거가 아직 약해. 다음 실험에서 **gradient covariance spectrum, Muon orthogonalization 전/후 singular spectrum, update–gradient cosine, layer별 update-to-weight**를 성공/실패 세팅에서 비교하면 이 연결고리까지 닫을 수 있어.
