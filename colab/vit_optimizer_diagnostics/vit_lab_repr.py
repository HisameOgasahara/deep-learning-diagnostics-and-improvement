...
def extract_features_and_logits(model, loader, device):
    ...
    for x, y in loader:
        ...
        for name in FEATURE_LAYERS:
            feature_chunks[name].append(
                features[name].float().cpu()
            )
...
def effective_rank_from_eig(eigenvalues, eps=1e-12):
    probabilities = (
        eigenvalues
        / (eigenvalues.sum() + eps)
    )
...
def class_geometry_metrics(features, logits, labels, head_weight):
    ...
    for class_id in classes:
        ...
        eigenvalues = covariance_spectrum(X_class)
        participation_dim = (
            eigenvalues.sum() ** 2
            / (np.sum(eigenvalues ** 2) + 1e-12)
        )
        class_dims.append(float(participation_dim))

    class_means = np.stack(class_means)
    centered_means = class_means - global_mean

    between_trace = np.mean(
        np.sum(centered_means ** 2, axis=1)
    )
    nc1 = (
        within_trace
        / (between_trace * len(classes) + 1e-12)
    )

    normalized_means = centered_means / (
        np.linalg.norm(
            centered_means,
            axis=1,
            keepdims=True,
    ...
    nc2_etf_error = float(
        np.sqrt(
            np.mean(
                (
                    gram[off_diagonal_mask]
                    - target_offdiag
                ) ** 2
            )
        )
    )
    center_abs_corr = float(
        np.mean(
            np.abs(gram[off_diagonal_mask])
        )
    )
    ...
    nc3_alignment = float(
        np.mean(
            np.sum(W * normalized_means, axis=1)
        )
    )
    ...
    return {
        ...
        "margin_positive_fraction": float(
            np.mean(margins > 0)
        ),
        "knn_purity": knn_purity,
        "mean_class_radius": float(
            np.mean(class_radii)
        ),
        "mean_class_participation_dim": float(
            np.mean(class_dims)
        ),
...
def representation_diagnostics(
    ...
    for run_name in optimizer_names:
        for epoch in diag_epochs:
            ...
            for layer in FEATURE_LAYERS:
                eigenvalues = covariance_spectrum(
                    val_features[layer]
                )
                ...
                representation_rows.append(
                    {
                        ...
                        "layer": layer,
                        "effective_rank": (
                            effective_rank_from_eig(
                                eigenvalues
                            )
                        ),
                        "cka_to_init": linear_cka(
                            init_val_features[layer],
                            val_features[layer],
                        ),
    ...
    representation_df.to_csv(
        csv_dir / "representation_summary.csv",
        index=False,
    )
    spectrum_df.to_csv(
        csv_dir / "covariance_spectrum.csv",
        index=False,
    )
    class_df.to_csv(
        csv_dir / "class_geometry.csv",
        index=False,
    )
