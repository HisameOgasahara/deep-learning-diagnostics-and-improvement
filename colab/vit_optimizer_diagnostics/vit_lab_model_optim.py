...
def zeropower_via_newton_schulz5(G, steps=5, eps=1e-7):
    """T4-safe Newton-Schulz orthogonalization path."""
    assert G.ndim == 2
...
class LocalMuon(torch.optim.Optimizer):
    """Single-GPU Muon fallback for runtimes where native Muon is unsuitable."""
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                update = update * math.sqrt(
                    max(1.0, p.shape[0] / p.shape[1])
                )
